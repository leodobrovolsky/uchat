...
char *mx_get_column_type(t_table *table, char *name) {
    t_table *tmp = table;
...
void mx_update_data(t_database t_db, char *condition) {
    ...
    request = mx_get_request_update(t_db, condition);
    res = sqlite3_open(t_db.name_db, &db);
    if (res!= SQLITE_OK) 
        mx_print_error("Cannot open database", 1, true);
    else
        res = sqlite3_exec(db, request, 0, 0, &err_msg);
...
int main (int argc, char *argv[]) {
    ...
    printf("%d\n", mx_check_unique_update_data(t_db, unique, 3));
    t_db.table = table1;
